...
class ExpectationMaximizationWithPriors(ParameterEstimator):
    def __init__(self, model, data, **kwargs):
        ...
        super(ExpectationMaximizationWithPriors, self).__init__(model, data, **kwargs)
    ...
    def get_parameters(
        ...
        for node in self.model_copy.nodes():
            cpd = self.model_copy.get_cpds(node)
            if cpd is not None:
                print(f'Using provided prior for {node}.')
                cpds.append(cpd)
            else:
